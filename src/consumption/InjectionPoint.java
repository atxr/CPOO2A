...
import consumption.devices.*;
import simulation.*;
import type.Type1;
/** Injection Point class, that represents an injection point with a list of devices */
public class InjectionPoint extends Simulation {
     ...
     * @param name
     */
    public InjectionPoint(String name) {
        devices = new ArrayList<>();
        this.name = name;
        addDevice(new Heating("Heating", 1));
        addDevice(new ContinuousDevice("Fridge", 0.2f));
        addDevice(new Device("TV", new Type1(List.of(19*60, 4*60, 2*60), List.of(0f, 0.1f, 0f))));
        addDevice(new Device("Computer", new Type1(List.of(8*60, 12*60, 4*60), List.of(0f, 0.1f, 0f))));
        addDevice(new Device("Kitchen", new Type1(List.of(12*60, 60, 7*60, 60, 3*60), List.of(0f, 10f, 0f, 10f, 0f))));
    }
